...
## While Claude builds: keep Supabase and the team in the loop

The ten provisioning steps report their own progress. The part after them, the
agent work, used to write nothing anywhere, so the dashboard showed a site at
`live` with zero pages and no sign that anyone was on it. Two people could claim
the same site and neither would know.

`npm run site` closes that. Call it around the build, not after it:

| when | command | what it does |
|---|---|---|
| before writing anything | `npm run site -- start <slug>` | marks `status=building`, emails the team that the site is claimed |
| after each page lands | `npm run site -- sync <slug>` | recounts pages, sections and images into Supabase |
| when the build is green | `npm run site -- done <slug>` | marks `status=built`, final counts, emails the team |
| when you give up | `npm run site -- fail <slug> "why"` | marks `status=failed`, emails the reason |

Two behaviours worth knowing, because both come from real confusion:

**A site with no row gets one.** `start` creates the `sites` row instead of
refusing. A site built from a bare clone still belongs on the dashboard, and a
missing row is not a reason to build in the dark. Everything the build learns
(pages, sections, images, status) ends up in Supabase either way.

**"Deployed but no pages yet" is its own email.** When `is_deployed` is true and
`page_count` is 0, provisioning has already put a real domain online showing the
holding page. A colleague opening that URL cannot tell whether it is abandoned or
claimed, so `start` says so in the subject line and the body.

Email goes out through Resend (`RESEND_API_KEY`, `NOTIFY_TO`, `NOTIFY_FROM` in
`.env`). No key means the Supabase write still happens and the email is skipped,
never a failed build.

---

## When the brief is thin, design rather than stall

**A missing layout is not a blocker.** No saved layout at `/sections`, no
`sectionOrder`, a scrape that returned little: build it anyway. Read the source
pages, decide the chronology yourself, and compose from the library
(`components/sections/**`, 139 blocks across Navbar, Hero, Features, Tours,
Testimonials, FAQ, CTA, Gallery, Logos, Blog, About, Team, Contact, Quote, Fleet,
plus six whole-page templates in `components/templates/`). Say in the handoff
which order you chose and why.

**The library is the floor, not the ceiling.** Composing entirely from stock
blocks is how a site ends up looking like every other site from this factory.
Author at least one section that exists only for this client: the thing they do
that nobody else does, given a layout nothing in the library covers. Put it in
`components/sections/<category>/` with the rest and it becomes available to the
next build.

**Scraped content that does not fit a block gets its own component.** A tide
table, a dive-site map, a boat comparison, a seasonal calendar, a menu, a price
matrix by group size: when the source has something real and no block holds it,
build the component instead of flattening it into a three-column card grid.
Losing the specific thing a client actually offers is a worse failure than an
unfamiliar layout. Same rules as everything else: copy from `content/*.ts`,
tokens for colour, plain lazy `<img>` for bulk imagery, `Reveal` for entrances.
...
Supabase, no re-scrape) · `npm run site -- start <slug>` (claim it, tell the team) ·
`npm run go` (deps, brand, guard, build, dev server) ·
`npm run site -- done <slug>` (final counts to Supabase, tell the team).
...
scripts/       pull.mjs (fetch from Supabase) · site-state.mjs (status, counters, email)
               clone.mjs (scrape) · apply-brand.mjs
